...
When the end points don’t move, the rope becomes static – i.e., basically
...
the curve will be deeper, but the length of the curve (called catenary)
...
- [dynamic] `EnableMovement` (default: off):

- [dynamic] `RopeStiffness` (positive number):

- [dynamic] `LinearDamping` (positive number):

- `MaxStretchFactor` (default: 1.1)

- [dynamic] `OutsideForce` (Vector3): -z: gravity; +z lift; x,y - wind.

- [dynamic] `DisableAngularMovement` (on/off):

- [dynamic] `AngularDamping` (positive number):

- [dynamic] `AngularInertia` (positive number):

- [dynamic] `DisableAngularMovement` (default: off):
