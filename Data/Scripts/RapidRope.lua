...
local endObjVelocity = V3_ZERO
local endObjAngularVelocity = V3_ZERO
local endObjQuat = Q_ZERO
-- static optimization
local dirtyFlag = true
local prevStartObjPosW = nil
local prevEndObjPosW = nil
...
-----------------------------
function Tick(dt)
    local startObjPosW = START_OBJECT:GetWorldPosition()
    local endObjPosW = END_OBJECT:GetWorldPosition()

    -- static optimization
    local isStatic = not PROPERTY_VALUES.Move
    if isStatic and not dirtyFlag then
        if prevEndObjPosW == endObjPosW and prevStartObjPosW == startObjPosW then
            return
        end
    end
    -- script is a parent for segments, set in to start
    script:SetWorldPosition(startObjPosW)

    local offset = endObjPosW - startObjPosW
    local offsetSize = offset.size
    local ropeLength = PROPERTY_VALUES.RopeLength

    -- endObj movement
    if PROPERTY_VALUES.Move then

        local damping = PROPERTY_VALUES.Damping
        damping = damping >= 0 and damping or 0 -- zero damping is ok
        local dL = offsetSize - PROPERTY_VALUES.RopeLength
        local ropeForceSize = dL > 0 and -dL*PROPERTY_VALUES.Stiffness or -MIN_ROPE_FORCE_MAGNITUDE
        local ropeForce = ropeForceSize * offset:GetNormalized()
        local acceleration = ropeForce - endObjVelocity*damping + PROPERTY_VALUES.OutsideForce
        endObjVelocity = endObjVelocity + dt*acceleration
        endObjPosW = endObjPosW + dt*endObjVelocity
        offset = endObjPosW - startObjPosW
        ...
        if offsetSize > maxStretchFactor*ropeLength then
           ...
           endObjPosW = startObjPosW + offset
        end
        END_OBJECT:SetWorldPosition(endObjPosW)

        -- endObj rotation
        if PROPERTY_VALUES.Rotate then
            local toCenter = endObjQuat:GetUpVector()
            local torque = -toCenter^ropeForce - PROPERTY_VALUES.RotationDamping * endObjAngularVelocity
            ...
            endObjAngularVelocity = endObjAngularVelocity + dt*angularAcceleration
            endObjQuat = Quaternion.New(endObjAngularVelocity, dt)*endObjQuat
            local rotation = Rotation.New(endObjQuat)
            -- normalization, without this we got some artifacts
            endObjQuat = Quaternion.New(rotation)
    ...
    prevStartObjPosW = startObjPosW
    prevEndObjPosW = endObjPosW
    dirtyFlag = false
end
